...
class CloudTransaction : public TransactionBaseImpl {
 public:
  CloudTransaction(DBCloud* db, const WriteOptions& write_options,
                         const TransactionOptions& txn_options,
                         const bool init = true);
  // No copying allowed
  CloudTransaction(const CloudTransaction&) = delete;
  void operator=(const CloudTransaction&) = delete;

  ~CloudTransaction() override;

  void Reinitialize(DBCloud* txn_db, const WriteOptions& write_options,
                    const TransactionOptions& txn_options);
 ...
 private:
  ...
  Status CommitWithoutPrepareInternal() ;

  Status CommitBatchInternal(WriteBatch* batch, size_t batch_cnt = 0) ;
