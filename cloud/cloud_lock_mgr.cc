...
std::shared_ptr<CloudLockManager> NewLockManager(DBCloudImpl* db,
  ...
  //  return std::shared_ptr<CloudLockManager>(opt.lock_mgr_handle, mgr);
  //} else {
    // Use a point lock manager by default
    return std::shared_ptr<CloudLockManager>(new PointCloudLockManager(db, opt));
  //}
}
...
PointCloudLockManager::PointCloudLockManager(DBCloudImpl* txn_db,
                                   const TransactionDBOptions& opt)
    : txn_db_impl_(txn_db),
...
void PointCloudLockManager::AddColumnFamily(const ColumnFamilyHandle* cf) {
...
void PointCloudLockManager::RemoveColumnFamily(const ColumnFamilyHandle* cf) {
...
std::shared_ptr<LockMap> PointCloudLockManager::GetLockMap(
...
bool PointCloudLockManager::IsLockExpired(TransactionID txn_id,
...
Status PointCloudLockManager::TryLock(CloudTransaction* txn,
                                 ColumnFamilyId column_family_id,
                                 const std::string& key, Env* env,
                                 bool exclusive) {
  // Lookup lock map for this column family id
  printf("column_family_id: %d, lock_maps_ size: %d\n", column_family_id, (int)lock_maps_.size());
...
Status PointCloudLockManager::AcquireWithTimeout(
    CloudTransaction* txn, LockMap* lock_map, LockMapStripe* stripe,
  ...
  if (!result.ok() && timeout != 0) {
    ...
    do {
      ...
      TEST_SYNC_POINT("PointCloudLockManager::AcquireWithTimeout:WaitingTxn");
      if (cv_end_time < 0) {
        // Wait indefinitely
...
void PointCloudLockManager::DecrementWaiters(
    const CloudTransaction* txn,
    const autovector<TransactionID>& wait_ids) {
  std::lock_guard<std::mutex> lock(wait_txn_map_mutex_);
  DecrementWaitersImpl(txn, wait_ids);
}

void PointCloudLockManager::DecrementWaitersImpl(
    const CloudTransaction* txn,
    const autovector<TransactionID>& wait_ids) {
  auto id = txn->GetID();
...
bool PointCloudLockManager::IncrementWaiters(
    const CloudTransaction* txn,
...
Status PointCloudLockManager::AcquireLocked(LockMap* lock_map, LockMapStripe* stripe,
                                       const std::string& key, Env* env,
...
void PointCloudLockManager::UnLockKey(CloudTransaction* txn,
...
void PointCloudLockManager::UnLock(CloudTransaction* txn,
                              ColumnFamilyId column_family_id,
                              const std::string& key, Env* env) {
  std::shared_ptr<LockMap> lock_map_ptr = GetLockMap(column_family_id);
  LockMap* lock_map = lock_map_ptr.get();
...
PointCloudLockManager::PointLockStatus PointCloudLockManager::GetPointLockStatus() {
...
std::vector<DeadlockPath> PointCloudLockManager::GetDeadlockInfoBuffer() {
  return dlock_buffer_.PrepareBuffer();
}

void PointCloudLockManager::Resize(uint32_t target_size) {
  dlock_buffer_.Resize(target_size);
}

PointCloudLockManager::RangeLockStatus PointCloudLockManager::GetRangeLockStatus() {
  return {};
}
...
Status PointCloudLockManager::TryLock(CloudTransaction* /* txn */,
  ...
  return Status::NotSupported(
      "PointCloudLockManager does not support range locking");
}
