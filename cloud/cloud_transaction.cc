...
std::atomic<TransactionID> CloudTransaction::txn_id_counter_(1);

TransactionID CloudTransaction::GenTxnID() {
  return txn_id_counter_.fetch_add(1);
}

CloudTransaction::CloudTransaction(
    DBCloud* txn_db, const WriteOptions& write_options,
    const TransactionOptions& txn_options, const bool init)
    : TransactionBaseImpl(
          txn_db->GetRootDB(), write_options,
          static_cast_with_check<DBCloudImpl>(txn_db)
  ...
  txn_db_impl_ = static_cast_with_check<DBCloudImpl>(txn_db);
...
void CloudTransaction::Initialize(const TransactionOptions& txn_options) {
  txn_id_ = GenTxnID();
...
void CloudTransaction::Clear() {
  txn_db_impl_->UnLock(this, *tracked_locks_);
  TransactionBaseImpl::Clear();
}

void CloudTransaction::Reinitialize(
    DBCloud* txn_db, const WriteOptions& write_options,
...
bool CloudTransaction::IsExpired() const {
  if (expiration_time_ > 0) {
...
Status CloudTransaction::GetForUpdate(const ReadOptions& read_options,
...
inline Status CloudTransaction::GetForUpdateImpl(
...
Status CloudTransaction::Put(ColumnFamilyHandle* column_family,
...
Status CloudTransaction::PutUntracked(ColumnFamilyHandle* column_family,
                                       const Slice& key, const Slice& value) {
  return Operate(
...
Status CloudTransaction::Delete(ColumnFamilyHandle* column_family,
...
Status CloudTransaction::DeleteUntracked(ColumnFamilyHandle* column_family,
                                          const Slice& key) {
...
Status CloudTransaction::SingleDelete(ColumnFamilyHandle* column_family,
                                       const Slice& key,
                                       const bool assume_tracked) {
  const bool do_validate = !assume_tracked;
  return Operate(column_family, key, do_validate, assume_tracked,
...
Status CloudTransaction::SingleDeleteUntracked(
...
Status CloudTransaction::Merge(ColumnFamilyHandle* column_family,
                                const Slice& key, const Slice& value,
                                const bool assume_tracked) {
  const bool do_validate = !assume_tracked;
  return Operate(column_family, key, do_validate, assume_tracked,
...
Status CloudTransaction::Operate(ColumnFamilyHandle* column_family,
...
Status CloudTransaction::SetReadTimestampForValidation(TxnTimestamp ts) {
...
Status CloudTransaction::SetCommitTimestamp(TxnTimestamp ts) {
  if (read_timestamp_ < kMaxTxnTimestamp && ts <= read_timestamp_) {
    return Status::InvalidArgument(
...
Status CloudTransaction::CommitBatch(WriteBatch* batch) {
...
Status CloudTransaction::Prepare() {
  if (name_.empty()) {
    return Status::InvalidArgument(
...
Status CloudTransaction::PrepareInternal() {
...
Status CloudTransaction::Commit() {
...
Status CloudTransaction::CommitWithoutPrepareInternal() {
...
Status CloudTransaction::CommitBatchInternal(WriteBatch* batch, size_t) {
  uint64_t seq_used = kMaxSequenceNumber;
...
Status CloudTransaction::CommitInternal() {
  WriteBatchWithIndex* wbwi = GetWriteBatch();
  assert(wbwi);
  WriteBatch* wb = wbwi->GetWriteBatch();
  assert(wb);
...
Status CloudTransaction::Rollback() {
  Status s;
...
Status CloudTransaction::RollbackInternal() {
...
Status CloudTransaction::RollbackToSavePoint() {
...
Status CloudTransaction::LockBatch(WriteBatch* batch,
...
// the snapshot time.
Status CloudTransaction::TryLock(ColumnFamilyHandle* column_family,
...
Status CloudTransaction::GetRangeLock(ColumnFamilyHandle* column_family,
...
Status CloudTransaction::ValidateSnapshot(
    ColumnFamilyHandle* column_family, const Slice& key,
    SequenceNumber* tracked_at_seq) {
...
bool CloudTransaction::TryStealingLocks() {
  assert(IsExpired());
  TransactionState expected = STARTED;
...
void CloudTransaction::UnlockGetForUpdate(
    ColumnFamilyHandle* column_family, const Slice& key) {
  txn_db_impl_->UnLock(this, GetColumnFamilyID(column_family), key.ToString());
}

Status CloudTransaction::SetName(const TransactionName& name) {
  Status s;
  if (txn_state_ == STARTED) {
    if (name_.length()) {
